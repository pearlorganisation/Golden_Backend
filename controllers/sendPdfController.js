...
if(name && email && mobileNumber && selectedPdfUrl){
  try {
    const data = await createAndSendPdfMail(pdfUrl,isAll,email, name, mobileNumber) 
    console.log("-----------the result is", data)
    // if(!data){

    // }
    return res.status(201).json({success: true, message:"Emails are sent "})
  }catch (error) {
    console.log("the error is",error)
    throw error    
  }
}else{
    return res.status(401).json({success:false, message:"Please send all the details"})
}
})